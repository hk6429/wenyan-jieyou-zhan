...
function fakeDeps({ ink = 100, mastery = {}, rng = () => 0.5 } = {}) {
  ...
  return {
    ...
    mastery: (tid) => mastery[tid] || { ratio: 0, total: 0 },
    title: (tid) => tid,
    author: () => '',
  };
}
...
test('合契失敗：只扣墨錠、回饋少量墨錠、不出文魄、雙親不動', () => {
  const f = FS.defaultFusion();
  const mastery = both({ ratio: 0.9, total: 40 });
  const deps = fakeDeps({ ink: 100, mastery, rng: () => FS.FAIL_RATE - 0.01 }); // < FAIL_RATE → 失敗
  const r = FS.fuse(f, 'yinyi', deps);
  assert.equal(r.ok, true);
  assert.equal(r.result, 'fail');
  assert.ok(FS.FAIL_LINES.includes(r.line));
  assert.equal(r.inkBack, FS.CONSOLE_INK);
  assert.equal(deps.getInk(), 100 - FS.FUSE_COST + FS.CONSOLE_INK); // 扣費後回饋安慰墨錠
  assert.equal(FS.listWenpo(f).some((w) => w.owned), false);        // 沒出文魄
});
