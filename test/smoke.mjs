...
// 新手收斂（③）：未精通任何一篇時，進階「江湖」子導覽（草堂/擂台/合契/市集）應隱藏
const subHiddenNew = await page.evaluate(() => {
  const sub = document.querySelector('nav.tabs-sub');
  return sub && getComputedStyle(sub).display === 'none';
});
if (!subHiddenNew) errors.push('新手期「江湖」子導覽未隱藏（③收斂失效）');

// 精通一篇（四題型各 3 題全對）後，江湖子導覽應解鎖顯示 → 才能續測所有進階分頁
await page.evaluate(() => {
...
if (!subShown) errors.push('精通一篇後「江湖」子導覽仍未顯示（③解鎖失效）');
