...
test('精通廣度看各題型答對數：theme 全錯時即使總答對率夠也不得精通', () => {
  const S = loadStore();
  for (const ty of ['char', 'sentence', 'gist']) for (let i = 0; i < 4; i++) S.recordAnswer('t03', true, ty, { qId: `${ty}-ok-${i}` });
  for (let i = 0; i < 2; i++) S.recordAnswer('t03', false, 'theme', { qId: `theme-bad-${i}` });
  const t = S.getTextState('t03');
  assert.ok(t.correct / t.total >= 0.8);
  assert.equal(t.types.theme.correct, 0);
  assert.equal(t.mastered, false);
});

test('精通新增逐題型正確率門檻：任一題型低於 60% 即使總正確率達 80% 仍不得精通', () => {
  const S = loadStore();
  for (let i = 0; i < 4; i++) S.recordAnswer('t03', i < 2, 'char', { qId: `char-floor-${i}` }); // 2/4 = 50%
  for (const ty of ['sentence', 'gist', 'theme']) {
    for (let i = 0; i < 4; i++) S.recordAnswer('t03', true, ty, { qId: `${ty}-floor-${i}` });
  }
  const t = S.getTextState('t03');
  assert.ok(t.correct / t.total >= 0.8);
  assert.equal(t.types.char.correct, 2);
  assert.equal(t.mastered, false);
});
...
test('每日墨錠上限仍生效（答對賺取被截斷）', () => {
  const S = loadStore();
  for (let i = 0; i < 250; i++) S.recordAnswer('t09', true, 'char', { qId: `k${i}` });
  assert.equal(S.inkToday().earned, S.DAILY_INK_CAP);
  assert.equal(S.inkToday().left, 0);
});

test('提示券是純消耗：30 墨購買、使用後減一，不改精通統計', () => {
  const S = loadStore();
  S.addInk(60);
  assert.equal(S.buyHintTicket(), true);
  assert.equal(S.getInk(), 30);
  assert.equal(S.getHintTickets(), 1);
  assert.equal(S.useHintTicket(), true);
  assert.equal(S.getHintTickets(), 0);
  assert.equal(S.allMastered().length, 0);
});

test('硯靈等級只綁真實 totalCorrect，續升至 Lv10 並封頂', () => {
  const S = loadStore();
  for (let i = 0; i < 240; i++) S.recordAnswer('t01', true, 'char', { qId: `lv-${i}` });
  assert.equal(S.totalCorrect(), 240);
  assert.equal(S.companionLevel(), 10);
});

test('錯題精通率只在真實答錯後再答對時提升', () => {
  const S = loadStore();
  S.recordAnswer('t01', false, 'char', { qId: 'fix-me' });
  assert.deepEqual(S.correctionStats(), { seen: 1, resolved: 0, ratio: 0 });
